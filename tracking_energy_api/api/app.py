...
async def energy_calculation():
    ...
    date = request.args.get('date') or datetime.strftime(
...
async def energy_history():
    ...
    response = final_energy_level or 0
    return jsonify(response)
...
async def time_prediction():
    ...
    prediction = {"minutes": floor(predict_time_to_threshold(
        energy_levels, threshold))} if energy_levels is not None else 0
...
async def energy_prediction():
    ...
    db = client.db
    energy_levels = await sync_to_async(energy_level_of_player_in_game)(db, ssiId, date, recovery_factor=0.2, calory_start=2000, cum=True)
    prediction = predict_energy_after_minutes(
        energy_levels, minutes) if energy_levels is not None else 0
    return jsonify(prediction)
...
def add_tracking_data():
    ...
    date = content.get("date") or datetime.strftime(
        datetime.now().date(), "%Y-%m-%d")
    period = content["period"]
    time = content["gameClock"]
    # consider having a different collection for home players, maybe even a different collection for each team
    players = content["homePlayers"]+content["awayPlayers"]
    ids_speeds = dict([(player["playerId"], player["speed"])
    ...
    for id, speed in ids_speeds.items():
        ...
        update = {"$push": {"speeds.period_1": {"time": time, "speed": speed}}} if period == 1 else {
            "$push": {"speeds.period_1": {"time": time, "speed": speed}}}
    ...
    return jsonify(f"{modified} modified documents")
